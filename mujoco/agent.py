...
class SacAgent:
    ...
    def __init__(self, env, log_dir, num_steps=3000000, batch_size=256,
        ...
        np.random.seed(seed)
        self.env.seed(seed)
        torch.backends.cudnn.deterministic = True  # It harms performance but good for reproducing
    ...
    def calc_target_q(self, states, actions, rewards, next_states, dones):
        with torch.no_grad():
            ...
            # the orinal paper used two Q values and use the smaller one as the real Q
            next_q1, next_q2 = self.critic_target(next_states, next_actions)
            next_q = torch.min(next_q1, next_q2) + self.alpha * next_entropies # 对应公式4.(1).3

        target_q = rewards + (1.0 - dones) * self.gamma_n * next_q # 对应公式4.(3).1的两个减号部分
    ...
    def calc_critic_loss(self, batch, weights):
        ...
        q1_loss = torch.mean((curr_q1 - target_q).pow(2) * weights) # 对应公式4.(3).1
    ...
    def calc_policy_loss(self, batch, weights):
        ...
        # 对应公式4.(3).-3，其是从KL散度推出来的，KL散度要尽量让我们使用高斯分布近似的policy
        # 与exp(Q)成正比（即接近exp(Q)的分布），求导的时候Z忽略了，所以得到公式4.(3).-3的形式。
        # log项在求了Expectation之后就是负的entropy了，所以minimize(-αE-Q)其实就是maximize (Q + alpha * entropy)
