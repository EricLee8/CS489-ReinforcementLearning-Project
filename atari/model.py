...
class LSTM_DQN(nn.Module):
    def __init__(self, h, w, outputs, device):
        ...
        self.conv2 = nn.Conv2d(32, 64, kernel_size=4, stride=2, bias=False)
        self.conv3 = nn.Conv2d(64, 64, kernel_size=3, stride=1, bias=False)
        self.LSTM = nn.LSTM(64*7*7, 512, num_layers=1, batch_first=True)
        self.fc2 = nn.Linear(512, outputs)
        self.device = device
    ...
    def forward(self, x):
        x = x.to(self.device).float() / 255. # (N, 4, 84, 84)
        x = x.unsqueeze(2) # (N, 4, 1, 84, 84)
        x = x.view(-1, 1, 84, 84) # (N*4, 1, 84, 84)
        x = F.relu(self.conv1(x))
        x = F.relu(self.conv2(x))
        x = F.relu(self.conv3(x)) # (N*4, 64, 7, 7)
        x = x.view(-1, 4, 64*7*7)
        _, (hn, _) = self.LSTM(x)
        hn = hn.view(-1, 512) # (N, 512)
        return self.fc2(hn)
...
class ReplayMemory(object):
    ...
    def push(self, state, action, reward, done):
        """Saves a transition."""
        self.m_states[self.position] = state # state: (1,5,84,84), push进去后就是(5, 84, 84)...
    ...
    def sample(self, bs):
        ...
        bs = self.m_states[i, :4] # state
        bns = self.m_states[i, 1:] # next state
